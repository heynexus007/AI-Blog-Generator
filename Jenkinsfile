pipeline {
  ...
  environment {
    ...
    ECR_REPO = "${AWS_ACCOUNT_ID}.dkr.ecr.${AWS_REGION}.amazonaws.com/ai-blog-ecr"
    ECS_CLUSTER = 'ai-blog-cluster'
    ECS_SERVICE = 'ai-blog-service'
    ECS_TASK_FAMILY = 'ai-blog-task'
  }
  ...
  post {
    success {
      echo "Successfully deployed to ECS: ${BUILD_NUMBER}"
    }
    failure {
